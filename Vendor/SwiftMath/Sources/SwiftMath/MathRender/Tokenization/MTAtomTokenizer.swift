...
class MTAtomTokenizer {
    ...
    private func tokenizeSpace(_ atom: MTMathAtom, prevAtom: MTMathAtom?, atomIndex: Int) -> MTBreakableElement? {
        ...
        // Indium: honor the command's own amount (\, \: \; \quad \qquad \!), in mu.
        let width: CGFloat
        if let space = atom as? MTMathSpace, let mu = font.mathTable?.muUnit {
            width = space.space * mu
        } else {
            width = widthCalculator.measureSpace(atom.type)
        }

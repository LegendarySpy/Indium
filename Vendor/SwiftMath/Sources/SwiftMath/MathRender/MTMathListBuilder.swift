    ...
    public mutating func buildInternal(_ oneCharOnly: Bool, stopChar stop: Character?) -> MTMathList? {
        ...
        while self.hasCharacters {
            ...
            if char == "^" {
                ...
                // Indium: inside \text{} a colon, comma or hyphen is prose, not a relation
                // or operator, so it takes no math spacing ("Zn:" not "Zn :").
                if spacesAllowed && currentFontStyle == .roman, let a = atom,
                   [.relation, .binaryOperator, .punctuation, .unaryOperator].contains(a.type) {
